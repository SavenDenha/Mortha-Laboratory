...
filtered_data <- combined_data %>%
    filter(Phenotype %in% c("CD3", "B220"))
...
summary_distances_combined <- filtered_data %>%
    ...
    summarize(
        ...
        Std_Distance = sd(`Distance to KLRG1`, na.rm = TRUE)
    )
print(summary_distances_combined)

# Create a single density plot for Distance to KLRG1
combined_density_plot <- ggplot(filtered_data, aes(x = `Distance to KLRG1`, color = Condition, linetype = Phenotype)) +
    ...
    xlab("Distance to KLRG1 from:") +  # Add x-axis label here
    theme(
